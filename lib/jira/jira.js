...
class Jira {
	...
	constructor({ username, token, hostname } = {}) {
		...
		if(!this.username || !this.token || !this.hostname) {
			debug('[JIRA PR] Received inputs', require('actions-context').inputs);
	...
	do_transition({ key, transition }) {
		debug(`[JIRA PR] Initiating transtiion for ${key} to ${transition}`);
		return this.request({
			method: 'POST',
	...
	get_transitions({ key }) {
		debug(`[JIRA PR] Getting transtiions for ${key}`);
		return this.request({
			path: `/rest/api/2/issue/${key}/transitions`
		});
	}
